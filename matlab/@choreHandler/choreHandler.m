...
classdef choreHandler
    ...
    methods
        ...
        function obj = choreHandler(config_file)
            ...
            for ii = 1:length(C)
                idx = find(ic == ii);
                obj.filepaths = string(fullfile({files(idx).folder},{files(idx).name}))';
        ...
        function obj = apply_time_filter(obj,tStart,tEnd,tDur)
            ...
            if ~exist('tEnd')
                tEnd = max(arrayfun(@(x) max([x.data_choreography(:).elapstime]), obj));
                tEnd = 120
            end
            if ~exist('tDur')
                tDur = 40;
...
%                 [time_filt{1} & et{1} <= [min(et{1}(time_filt{1}))+tDur]]
                ...
                filt2 = filt;
%                 range(et{1}(time_filt{1}))>(tDur-1)
                filt2(filt) = cellfun(@(x,y) range(x(y))>(tDur-1), et(filt), time_filt(filt));
                
%                 checker(filt2) = cellfun(@(x,y) x(y), et(filt2), time_filt(filt2), 'UniformOutput', false)
%                 cellfun(@range, checker, 'UniformOutput', false)
        ...
        %% Boxplots
        function [box_handle,spread_handle] = plot_boxplot(obj,metric_to_plot,params)
    ...
    methods (Static)
        ...
        function file_selection = get_filelist(params)
            files = dir(fullfile(params.directories.choreography_results,'**'));
