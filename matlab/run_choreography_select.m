...
function run_choreography_select(InputDirectory,OutputDirectory,params)
...
filenames = strrep(string({inpFiles.name}'),'.summary','');
[files_prompt,I] = sort(filenames,'descend');
[indx,tf] = listdlg('ListString',files_prompt);
ref = sort(I(indx));
inpFiles = inpFiles(ref);
...
%     redo = outlength ~= length(features)+1;
